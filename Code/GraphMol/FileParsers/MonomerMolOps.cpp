...
// Defined here (FileParsers layer) because it depends on MonomerLibrary, which
// is not visible from GraphMol.
MACROMol::MACROMol(bool useGlobalLibrary) {
  if (useGlobalLibrary) {
    setGlobalLibrary(
        &MonomerLibrary::getGlobalLibrary()->getMACROMolTemplateLib());
  }
}
...
bool isMonomer(const Atom *atom) {
  ...
  return atom->getPropIfPresent(RDKit::common_properties::dummyLabel,
                                dummyLabel) &&
         dummyLabel != "" &&
         atom->getPropIfPresent(RDKit::common_properties::molAtomClass,
                                atomClass) &&
         atomClass != "";
}
...
void setResidueNumber(Atom *atom, int residue_number) {
  ...
  if (monomer_info == nullptr) {
    throw std::runtime_error("Atom " + std::to_string(atom->getIdx()) +
                             " does not have monomer info");
  }
  monomer_info->setResidueNumber(residue_number);
}

bool isValidChain(const RDKit::MACROMol &monomer_mol,
...
Atom *findChainBegin(RDKit::MACROMol &monomer_mol) {
...
void orderResidues(RDKit::MACROMol &monomer_mol) {
...
std::vector<std::string> MACROMol::getPolymerIds() const {
...
Chain MACROMol::getPolymer(std::string_view polymer_id) const {
  ...
  for (const auto &sg : ::RDKit::getSubstanceGroups(*this)) {
    if ((sg.getProp<std::string>("TYPE") != "COP") || !sg.hasProp(ANNOTATION) ||
        !sg.hasProp("ID")) {
      continue;
    }
  ...
  for (const auto &ring : bnd_rings) {
    if (std::ranges::all_of(ring, [this](const auto &idx) {
      ...
      for (const auto &idx : ring) {
        ...
        int diff = getResidueNumber(begin_at) - getResidueNumber(end_at);
      ...
      if (connection_bond != getNumBonds()) {
        ...
        getBondWithIdx(connection_bond)->setProp(EXTRA_LINKAGE, linkage);
      } else {
