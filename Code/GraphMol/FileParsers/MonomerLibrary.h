...
struct RDKIT_FILEPARSERS_EXPORT MonomerEntry {
...
*/
class RDKIT_FILEPARSERS_EXPORT MonomerLibrary {
 public:
  //! Return type for getMonomerInfo: {symbol, original_data, monomer_class}
  using monomer_info_t =
  ...
  explicit MonomerLibrary()
      : d_localMacroMolTemplateLib(std::unique_ptr<RDKit::MACROMolTemplateLib>(
            new RDKit::MACROMolTemplateLib())),
  ...
  explicit MonomerLibrary(RDKit::MACROMolTemplateLib &libInit)
      : d_macroMolTemplateLib(libInit) {};
  ...
  /*!
      ...
      \return tuple of {symbol, original_data, monomer_class} or nullopt if not
     found
  */
  [[nodiscard]] monomer_info_t getMonomerInfo(
      const std::string &pdb_code, const std::string monomer_class) const;

  //! Get PDB code for a monomer symbol
  [[nodiscard]] std::optional<std::string> getPdbCode(
      const std::string &symbol, const std::string &monomer_class) const;

  [[nodiscard]] std::optional<std::vector<std::string>> getPdbCodes(
      const std::string &symbol, const std::string &monomer_class) const;
  ...
  [[nodiscard]] const MACROMolTemplate *getMonomer(
      const std::string &monomer_id, const std::string &monomer_class) const;
  ...
  void addMonomerFromSDF(const std::string &sdf_data, const std::string &symbol,
                         const std::string &monomer_class,
                         const std::string &pdb_code = "");

  //! Add a monomer with a pre-parsed molecule
  void addMonomer(std::unique_ptr<RWMol> &mol, const std::string &data,
 ...
 private:
  ...
  std::unique_ptr<RDKit::MACROMolTemplateLib>
      d_localMacroMolTemplateLib;  // used for standalone MACROMolTemplateLib
