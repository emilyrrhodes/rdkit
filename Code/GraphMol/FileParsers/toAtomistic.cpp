...
//                             unsigned int residue_num, unsigned int
//                             old_mol_size)
// {
//     for (const auto& atom : new_monomer.atoms()) {
//         unsigned int map_num;
//         if
//         (atom->getPropIfPresent(RDKit::common_properties::molAtomMapNumber,
//                                    map_num)) {
...
// void setPDBResidueInfo(RDKit::RWMol& new_monomer, const std::string&
// monomer_label,
...
//                          std::vector<unsigned int>& remove_atoms, char
//                          chain_id, const MonomerLibrary& db)
// {
//     // Maps residue number and attachment point number to the atom index in
//     // atomistic_mol that should be attached to and the atom index of the
//     rgroup
...
//                 // SHARED-11457
//                 new_monomer.reset(
//                     RDKit::SmilesToMol("[" + monomer_data + "]", 0,
//                     sanitize));
//             }

//             // SMILES monomers may be in rgroup form like
//             // *N[C@H](C(=O)O)S* |$_R1;;;;;;;_R3$| or use atom map numbers
//             like
...
//                         monomer_class + " monomer " + monomer_label + " not
//                         found in Monomer database");
//                 }
...
//         setPDBResidueInfo(*new_monomer, monomer_label, residue_number,
//         chain_id,
//                        monomer_class, db);
...
//         atomistic_mol.addBond(core_aid1, core_aid2, ::RDKit::Bond::SINGLE) -
//             1;
...
//         if (begin_attachment_points.find(std::make_pair(begin_res,
//         from_rgroup)) ==
...
//                 " between monomers " + std::to_string(begin_atom->getIdx()) +
//                 " and " + std::to_string(end_atom->getIdx());
//             throw std::runtime_error(error_msg);
//         }

//         auto [core_atom1, attachment_point1] =
//             begin_attachment_points.at(std::make_pair(begin_res,
//             from_rgroup));
//         auto [core_atom2, attachment_point2] =
//             end_attachment_points.at(std::make_pair(end_res, to_rgroup));
//         atomistic_mol->addBond(core_atom1, core_atom2,
//         ::RDKit::Bond::SINGLE); remove_atoms.push_back(attachment_point1);
//         remove_atoms.push_back(attachment_point2);
//     }
...
//     // Remove graph hydrogens where some RGroups previously were. This will
//     turn
//     // H - NH to NH2, etc
//     RDKit::MolOps::removeHs(*atomistic_mol);

//     // Remove atom map numbers -- anything remaining is a capping group and
//     the
...
std::unique_ptr<RDKit::RWMol> toAtomistic(const MACROMol &macro_mol) {
  ...
  return RDKit::MolFromMACROMol(&macro_mol, molFileParserParams,
