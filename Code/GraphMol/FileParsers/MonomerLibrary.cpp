...
std::unique_ptr<MACROMolTemplateLib>
    MonomerLibrary::s_globalMACROMOLTemplateLib;
...
const std::list<std::tuple<std::string, std::string, std::string, std::string>>
    builtin_monomer_data = {
        ...
        {"H", "PEPTIDE", "HIS/HIE/HIP/HID/HSE/HSD/HSP",
...
//             this->addTemplate(globalTemplate);   // NOT owned by this lib,
//             but still owned by the global lib
//         }
//     }
// }

// MonomerLibrary::MonomerLibrary ([[maybe_unused]] std::string_view
// database_path) {
...
//     for (auto globalTemplate : globalLib) {
//         this->addTemplate(globalTemplate);   // NOT owned by this lib, but
//         still owned by the global lib
//     }
// }
...
void MonomerLibrary::loadBuiltinDefinitions() {
  ...
  for (const auto &[symbol, monomerClass, pdbCodes, data] :
       builtin_monomer_data) {
    addMonomerFromSmiles(data, symbol, monomerClass, pdbCodes);
    // addMonomerFromSmiles(data, symbol, "PEPTIDE", symbol_to_pdb.count(symbol)
    // ? symbol_to_pdb.at(symbol) : "");
  ...
  //     }
  // }
  // Sort the monomer library by the number of atoms in the monomer, so that
  // when we search for monomers, we find the largest match first
}

std::optional<std::string> MonomerLibrary::getMonomerData(
    const std::string &monomer_id, const std::string &monomer_class) const {
  ...
  auto templateMol =
      this->getMACROMolTemplateLib().getTemplate(monomer_class, monomer_id);
...
MonomerLibrary::monomer_info_t MonomerLibrary::getMonomerInfo(
    const std::string &symbol, const std::string monomer_class) const {
  ...
  auto templateMol =
      this->getMACROMolTemplateLib().getTemplate(monomer_class, symbol);
  if (templateMol) {
    templateMol->getPropIfPresent("OriginalData", originalData);

    if (templateMol->getPropIfPresent(common_properties::templateNames,
                                      templateNames)) {
      return std::make_tuple(templateNames[0], originalData, monomer_class);
    }
  }
  return std::nullopt;
}

std::optional<std::string> MonomerLibrary::getPdbCode(
    const std::string &symbol, const std::string &monomer_class) const {
...
std::optional<std::vector<std::string>> MonomerLibrary::getPdbCodes(
    const std::string &symbol, const std::string &monomer_class) const {
  auto templateMol =
      this->getMACROMolTemplateLib().getTemplate(monomer_class, symbol);
  ...
  if (templateMol->getPropIfPresent("PDBCodes", pdbCodesStr)) {
    boost::algorithm::split(pdbCodes, pdbCodesStr,
...
const RDKit::MACROMolTemplate *MonomerLibrary::getMonomer(
    const std::string &monomer_id, const std::string &monomer_class) const {
  // MonomerKey key{monomer_id, monomer_class};
  // auto it = d_monomers.find(key);
  // if (it != d_monomers.end()) {
  ...
  return this->getMACROMolTemplateLib().getTemplate(monomer_class, monomer_id);
}
...
void MonomerLibrary::addMonomer(std::unique_ptr<RWMol> &templateMol,
  ...
  // go through the mol and find the attachments points.  They are the atoms
  // with atom map nums. add the connection point and leaving group for each one
  ...
  for (unsigned int atomIdx = 0; atomIdx < atomCount; ++atomIdx) {
    ...
    if (atom->getPropIfPresent(RDKit::common_properties::molAtomMapNumber,
      ...
      // see if this atom IS the leaving group.  It is if it in a R-atom (atomic
      ...
      if (atomicNumber == 0 || atomicNumber == 1 ||
        ...
        for (auto bnd : templateMol->atomBonds(atom)) {
          ...
          mainSgroup->addParentAtomWithIdx(
              otherAtomIdx);  // add to the main Sgroup

          // add a SUP substance group for the leaving group

          lvgSgroup->addAtomWithIdx(atom->getIdx());
          lvgSgroup->addBondWithIdx(bnd->getIdx());
          mainSgroup->addBondWithIdx(
              bnd->getIdx());  // add bond to the main Sgroup
        }
      } else {
        ...
        if (oldAtomicNumber == 8 /*oxygen*/ ||
            oldAtomicNumber == 7 /*nitrogen*/ ||
  ...
  auto newTemplate = std::unique_ptr<MACROMolTemplate>(new MACROMolTemplate(
      templateMol, monomer_class, templateNames, otherAttrs));
...
void MonomerLibrary::addMonomerFromSmiles(const std::string &smiles,
  ...
  //  };
  auto templateMol(
      std::unique_ptr<RDKit::RWMol>(SmilesToMol(smiles, 0, false)));
...
void MonomerLibrary::addMonomerFromSDF(const std::string &sdf_data,
  ...
  auto templateMol(
      std::unique_ptr<RDKit::RWMol>(MolBlockToMol(sdf_data, false, false)));
...
bool MonomerLibrary::hasMonomer(const std::string &symbol,
  ...
  return d_macroMolTemplateLib.contains(monomer_class, symbol);
}
...
MonomerLibrary *MonomerLibrary::getGlobalLibrary() {
  std::call_once(s_globalLibraryOnce, []() {
    ...
    s_globalLibrary =
        std::make_unique<MonomerLibrary>(*(s_globalMACROMOLTemplateLib.get()));
