...
/*
 * Temporary, simple FASTA parser to show how to use the MACROMol
 */
  ...
  for (auto chain_id : chains) {
    ...
    for (auto atom : chain.atoms) {
      ...
      const auto *res_info = static_cast<const RDKit::AtomPDBResidueInfo *>(
          mol.getAtomWithIdx(atom)->getMonomerInfo());
...
static bool same_roundtrip_mol(const RDKit::ROMol &original,
  ...
  params.maxMatches = 1;
  auto match = RDKit::SubstructMatch(roundtrip, original, params);
  if (match.empty()) {
    return false;
  }
...
TEST_CASE("FASTAConversions") {
  SECTION("SIMPLE") {
    // Build MACROMol with a single chain
    MACROMol monomer_mol;
  ...
  SECTION("MultipleChains") {
    // Build MACROMol with two chains
    MACROMol monomer_mol;
  ...
  SECTION("UsingSequenceReader") {
    ...
    // PDB info is used to convert the atomistic sturcture into a MACROMol
    auto monomer_mol = toMonomeric(*atomistic_mol);

    CHECK(std::string(">Chain PEPTIDE1\nCGCGAATTACCGCG") ==
          to_fasta(*monomer_mol));
...
TEST_CASE("Conversions") {
  ...
  SECTION("toAtomisticWithBranch") {
    ...
    CHECK(
        smi ==
        "C[C@H](N)C(=O)N[C@@H](CC(=O)N[C@@H](CS)C(=O)O)C(=O)N1CCC[C@H]1C(=O)O");
  }
  ...
  SECTION("toAtomisticCyclicPeptide") {
    std::string helm =
        "PEPTIDE1{F.Y.K.A.R.L}$PEPTIDE1,PEPTIDE1,6:R2-1:R1$$$V2.0";
    auto atomistic_mol = HELMToMol(helm);
  ...
  SECTION("toAtomisticSmilesMonomer") {
    // This is equivelent to HELM string
    // "PEPTIDE1{A.[O=C([C@H]1CCCN1[*:1])[*:2]].P}$$$$"
    MACROMol monomer_mol(true);
    auto midx1 = monomer_mol.addMonomer("A", 1, "PEPTIDE", "PEPTIDE1");
    auto midx2 = monomer_mol.addMonomer("O=C([C@H]1CCCN1[*:1])[*:2]",
                                        MonomerType::SMILES);
  ...
  SECTION("toMonomeric1DNG") {
    ...
    pdbfile += "/Code/GraphMol/FileParsers/test_data/1dng.pdb";
    ...
    CHECK(same_roundtrip_mol(*mol, *roundtrip,
                             monomer_mol->getPolymerIds().size()));
  }

  SECTION("toMonomeric2N65") {
    // Example with multiple chains and a disulfide bond between chains
    std::string pdbfile = getenv("RDBASE");
    pdbfile += "/Code/GraphMol/FileParsers/test_data/2n65.pdb";
    auto mol = std::unique_ptr<RWMol>(PDBFileToMol(pdbfile));
    auto monomer_mol = toMonomeric(*mol);
    ...
    neutralize_atoms(*roundtrip);
    CHECK(same_roundtrip_mol(*resolved_his, *roundtrip,
                             monomer_mol->getPolymerIds().size()));
  }
  ...
  SECTION("toMonomeric4QAF") {
    ...
    std::string pdbfile = getenv("RDBASE");
    pdbfile += "/Code/GraphMol/FileParsers/test_data/4qaf.pdb";
    auto mol = std::unique_ptr<RWMol>(PDBFileToMol(pdbfile));
  ...
  SECTION("toMonomeric5VAV") {
    ...
    std::string pdbfile = getenv("RDBASE");
    pdbfile += "/Code/GraphMol/FileParsers/test_data/5vav.pdb";
    auto mol = std::unique_ptr<RWMol>(PDBFileToMol(pdbfile));
    auto monomer_mol = toMonomeric(*mol);
    ...
    // Two extra bonds than atoms because this structure has two cycle closures,
    // one by a disulfide bond and another from an R2-R1 closure
