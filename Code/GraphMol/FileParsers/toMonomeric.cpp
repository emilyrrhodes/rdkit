...
void findChainsAndResidues(const RDKit::ROMol &mol,
  ...
  for (unsigned int i = 0; i < mol.getNumAtoms(); ++i) {
    ...
    if (!res_info) {
      ...
      throw std::runtime_error("Atom " + std::to_string(atom->getIdx()) +
                               " does not have residue info");
    }
    ...
    chains_and_residues[chain_id][std::make_pair(res_num, ins_code)].push_back(
        i);
  }
}

static void copy_selected_atoms_and_bonds(
...
std::unique_ptr<RDKit::RWMol> extractMolFragment(
  ...
  copy_selected_atoms_and_bonds(*extracted_mol, mol, selected_atoms,
                                selected_bonds);
  ...
  for (auto idx : atom_idxs) {
    ...
    for (const auto neigh : mol.atomNeighbors(at)) {
      ...
      if (key != current_key || res_info->getChainId() != chain_id) {
        ...
        // point
        attch_idxs.push_back({res_info->getResidueNumber(), at->getIdx()});
      }
    }
  }
  ...
  for (const auto &[_, ref_idx] : attch_idxs) {
    for (auto at : mol_fragment->atoms()) {
      ...
      if (at->getPropIfPresent(REFERENCE_IDX, orig_idx) &&
        ...
        mol_fragment->addBond(new_at_idx, at->getIdx(), RDKit::Bond::SINGLE);
        mol.getAtomWithIdx(orig_idx)->setProp<unsigned int>(MONOMER_MAP_NUM,
                                                            current_attchpt);
        ++current_attchpt;
      }
    }
    ...
    for (auto at : mol.atoms()) {
      ...
      if (at->getPropIfPresent(RDKit::common_properties::molAtomMapNumber,
        ...
        for (const auto &nbr : mol.atomNeighbors(at)) {
          nbr->setProp(RDKit::common_properties::molAtomMapNumber, map_no);
        }
  ...
  if (match.size()) {
    ...
    for (auto at : monomer_frag->atoms()) {
      ...
      if (it == match[0].end()) {
        ...
        boost::tie(nbrIdx, endNbrs) = monomer_frag->getAtomNeighbors(at);

        it = std::find_if(match[0].begin(), match[0].end(),
                          [nbrIdx](const std::pair<int, int> &p) {
                            return p.second == static_cast<int>(*nbrIdx);
                          });
        if (it == match[0].end()) {
          return false;
        }
        ...
        if (!db_atom->hasProp(RDKit::common_properties::molAtomMapNumber)) {
          return false;
        }
      }
    }
    ...
    for (auto [db_idx, at_idx] : match[0]) {
      ...
      unsigned int map_no;
      if (db_at->getPropIfPresent(RDKit::common_properties::molAtomMapNumber,
                                  map_no)) {
...
void detectLinkages(RDKit::MACROMol &macro_mol,
                    const RDKit::RWMol &atomistic_mol) {
  ...
  for (const auto &bond : atomistic_mol.bonds()) {
    ...
    unsigned int begin_monomer_idx = std::numeric_limits<unsigned int>::max();
    ...
    if (begin_monomer_idx == end_monomer_idx ||
        macro_mol.getBondBetweenAtoms(begin_monomer_idx, end_monomer_idx) !=
            nullptr) {
      continue;
    }

    const auto begin_monomer = macro_mol.getAtomWithIdx(begin_monomer_idx);
    const auto end_monomer = macro_mol.getAtomWithIdx(end_monomer_idx);
    ...
    if (begin_attchpt == -1 || end_attchpt == -1) {
      ...
      auto begin_res_info = dynamic_cast<const RDKit::AtomPDBResidueInfo *>(
    ...
    if (begin_attchpt == 2 && end_attchpt == 1) {
      macro_mol.addMacroBond(begin_monomer_idx, end_monomer_idx,
                             RDKit::Bond::SINGLE, "R2", "R1");
    } else if (begin_attchpt == 1 && end_attchpt == 2) {
      macro_mol.addMacroBond(end_monomer_idx, begin_monomer_idx,
                             RDKit::Bond::SINGLE, "R2", "R1");
    } else if (begin_monomer_idx < end_monomer_idx) {
      macro_mol.addMacroBond(begin_monomer_idx, end_monomer_idx,
                             RDKit::Bond::SINGLE,
                             "R" + std::to_string(begin_attchpt),
                             "R" + std::to_string(end_attchpt));
    } else {
      macro_mol.addMacroBond(end_monomer_idx, begin_monomer_idx,
                             RDKit::Bond::SINGLE,
                             "R" + std::to_string(end_attchpt),
...
MonomerLibrary::monomer_info_t getMonomerInfoFromAtom(const MonomerLibrary &db,
                                                      const RDKit::Atom *atom) {
  // This comes from something like a PDB or MAE file
  auto res_info =
      dynamic_cast<const RDKit::AtomPDBResidueInfo *>(atom->getMonomerInfo());
  auto res_name = res_info->getResidueName();
  res_name.erase(std::remove_if(res_name.begin(), res_name.end(), ::isspace),
                 res_name.end());
...
std::unique_ptr<RDKit::MACROMol> pdbInfoAtomisticToMM(
    const RDKit::ROMol &input_mol) {
  ...
  std::map<std::string, unsigned int> chain_counts = {
      {"PEPTIDE", 0}, {"RNA", 0}, {"DNA", 0}, {"CHEM", 0}};
  auto macro_mol = std::make_unique<RDKit::MACROMol>(true);

  // Access the monomer library
  const auto *db = MonomerLibrary::getGlobalLibrary();
  ...
  for (const auto &[chain_id, residues] : chains_and_residues) {
    ...
    std::string polymer_chain_id =
        monomer_class + std::to_string(++chain_counts[monomer_class]);
    // Assuming residues are ordered correctly
    unsigned int res_num = 1;
    for (const auto &[key, atom_idxs] : residues) {
      monomer_info =
          getMonomerInfoFromAtom(*db, mol.getAtomWithIdx(atom_idxs[0]));
      ...
      if (monomer_info &&
        ...
        // database
        this_monomer =
            macro_mol->addMacroAtom(monomer_class, std::get<0>(*monomer_info));
      } else {
        auto smiles = getMonomerSmiles(mol, atom_idxs, chain_id, key, res_num,
                                       end_of_chain);
        this_monomer = macro_mol->addMacroAtom(monomer_class, smiles);
      }
      // Copy AtomPDBResidueInfo from one of the atoms in this residue to the
      // monomer so we can access it later when we are determining linkages
...
std::unique_ptr<RDKit::MACROMol> toMonomeric(
  ...
  auto macro_mol = pdbInfoAtomisticToMM(atomistic_mol);
  // macro_mol->assignChains();
  return macro_mol;
}
...
//   RDKit::MolToMACROMol(monomerMol.get(), atomistic_mol,
//   templateLib->getMACROMolTemplateLib(),  molToMACROMolParams);
