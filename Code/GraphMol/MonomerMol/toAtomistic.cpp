...
void setPDBResidueInfo(RDKit::RWMol &new_monomer,
  ...
  std::string residue_name = (monomer_class == "PEPTIDE") ? "UNK" : "UNL";
...
AttachmentMap addPolymer(RDKit::RWMol &atomistic_mol,
  ...
  for (const auto monomer_idx : chain.atoms) {
    ...
    // Add atomistic components directly
    ...
    if (monomer->getProp<bool>(SMILES_MONOMER)) {
      ...
      if (mol) {
        ...
        if (!data) {
          throw std::out_of_range(monomer_class + " monomer " + monomer_label +
    ...
    fillAttachmentPointMap(*new_monomer, attachment_point_map, residue_number,
  ...
  for (const auto bond_idx : chain.bonds) {
    ...
    if (attachment_point_map.find(std::make_pair(from_res, from_rgroup)) ==
      ...
      throw std::runtime_error("Invalid linkage " +
                               bond->getProp<std::string>(LINKAGE) +
                               " between monomers " + std::to_string(from_res) +
                               " and " + std::to_string(to_res));
    }
    ...
    [[maybe_unused]] auto atomistic_bond_idx =
        atomistic_mol.addBond(core_aid1, core_aid2, ::RDKit::Bond::SINGLE) - 1;
...
std::unique_ptr<RDKit::RWMol> toAtomistic(const MonomerMol &monomer_mol) {
  ...
  for (auto at : atomistic_mol->atoms()) {
    // Only remove dummy atoms that are rgroup placeholders (atom map num != 0).
    // Atomistic atoms from a hybrid mol may also have atomicNum=0 and must
    // be preserved.
    if (at->getAtomicNum() == 0 && at->getAtomMapNum() != 0) {
