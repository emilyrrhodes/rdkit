...
void MonomerMol::addConnection(size_t monomer1, size_t monomer2,
  ...
  if (!isMonomer(getAtomWithIdx(monomer1)) ||
      !isMonomer(getAtomWithIdx(monomer2))) {
    throw std::runtime_error("addConnection must be called with two monomers.");
  }

  // if bond already exists, extend linkage information using 'extra_linkage'
  // property
  if (auto bond = getBondBetweenAtoms(monomer1, monomer2); bond != nullptr) {
    ...
    if (!bond->getPropIfPresent(LINKAGE, existing_linkage)) {
      throw std::runtime_error("No linkage property on bond between atom=" +
                               std::to_string(monomer1) +
                               " and atom=" + std::to_string(monomer2));
    }
    ...
    // For now, only set 'extra linkage' on linkages that already exist as a
    ...
    auto create_bond = [&, this](unsigned int first_monomer,
      ...
      const auto new_total =
          this->addBond(first_monomer, second_monomer, bond_type);
...
void MonomerMol::addAtomMonomerConnection(size_t atom_idx, size_t monomer_idx,
  ...
  if (!isMonomer(this->getAtomWithIdx(monomer_idx)) ||
      isMonomer(this->getAtomWithIdx(atom_idx))) {
    throw std::runtime_error(
...
size_t MonomerMol::addMonomer(std::string_view name, MonomerType monomer_type) {
  if (getNumAtoms() == 0) {
    throw std::invalid_argument("No atoms in molecule to determine chain ID");
  }
